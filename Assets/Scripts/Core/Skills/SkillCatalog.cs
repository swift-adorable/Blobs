...
/// <summary>
/// 전체 Skill 정의 목록을 담는 에셋.
///
/// 왜 Resources.LoadAll이 아니라 카탈로그 에셋인가:
/// v5 §10-10이 정의 에셋의 위치를 Assets/Data/ScriptableObjects/Skills/ 로 확정했다.
...
public class SkillCatalog : ScriptableObject
{
    ...
    public const string DefinitionFolder = "Assets/Data/ScriptableObjects/Skills";

    [Tooltip("SkillCatalogBuilder가 자동으로 채운다. 직접 편집하지 않는다.")]
    ...
    public List<SkillDefinition> GetByCategory(
        SkillCategory category, List<SkillDefinition> result = null)
    {
    ...
    {
        return Resources.Load<SkillCatalog>(ResourcePath);
    }

#if UNITY_EDITOR
    /// <summary>에디터 빌더 전용. 런타임에서는 호출되지 않는다.</summary>
    public void EditorSetDefinitions(List<SkillDefinition> source)
    {
        definitions.Clear();
