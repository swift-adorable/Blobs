...
namespace Blob.Tests
{
    ...
    {
        /// <summary>모든 자리가 열린 빌드. 합성 규칙 검증에 슬롯 개방은 변수가 아니다.</summary>
        ...
        public void 행동_Support와_부여_Core가_한_발에_합쳐진다()
        {
            ...
            Assert.IsTrue(build.TryEquipCore(AilmentCore("core_fire", SkillTag.Fire, StatusEffectType.Ignite), 0));
            Assert.IsTrue(build.TryEquipSupport(BehaviourSupport("sup_pierce", ProjectileBehaviourType.Pierce, 3), 0, 0));

            WeaponModifiers modifiers = build.GetModifiers();
        ...
        public void Core_2개여도_탄_수는_늘지_않는다()
        {
            ...
            build.TryEquipCore(AilmentCore("core_fire", SkillTag.Fire, StatusEffectType.Ignite), 0);
            build.TryEquipCore(AilmentCore("core_frost", SkillTag.Cold, StatusEffectType.Freeze), 1);

            build.TryEquipSupport(BehaviourSupport("sup_pierce", ProjectileBehaviourType.Pierce, 3), 0, 0);
            build.TryEquipSupport(BehaviourSupport("sup_split", ProjectileBehaviourType.Split, 1), 1, 0);

            Assert.AreEqual(1, build.GetModifiers().TotalProjectiles);
        }
        ...
        public void 탄_수는_다중_사격_계열로만_늘어난다()
        {
            ...
            Assert.IsTrue(build.TryEquipSupport(SkillTestFactory.CreateSupport(
                "sup_multishot", SkillTag.Projectile,
                cost: CostType.BarrageDensity,
                extraProjectiles: 2,
                fireIntervalMultiplier: 1.3f), 0, 0));
        ...
        [Test]
        public void 부여_Core_2개는_두_상태를_모두_싣는다()
        {
            SocketedBuild build = FullBuild();
        ...
        [Test]
        public void 부여_Core_2개는_발사마다_번갈아_부여된다()
        {
            SocketedBuild build = FullBuild();
        ...
        public void 기폭_계열_Core는_합성_대상이_아니다()
        {
            ...
            build.TryEquipCore(SkillTestFactory.CreateCore(
                "core_shockwave",
                SkillTag.AreaOfEffect | SkillTag.Detonator,
                family: CoreFamily.Detonation), 0);

            Assert.AreEqual(0, build.GetModifiers().Ailments.Count);
        }
        ...
        public void 기능_배타로_차단된_상태는_탄에_실리지_않는다()
        {
            ...
            Assert.AreEqual(1, build.GetModifiers().Ailments.Count);

            Assert.IsTrue(build.TryEquipSupport(SkillTestFactory.CreateSupport(
                "sup_burnt_offering", SkillTag.Fire,
                cost: CostType.FunctionalExclusion,
                blocksStatusCreation: true,
                blockedStatus: StatusEffectType.Ignite), 0, 0));
        ...
        public void 같은_상태를_만드는_Core가_겹쳐도_중복_등록되지_않는다()
        {
            ...
            build.TryEquipCore(AilmentCore("core_fire_a", SkillTag.Fire, StatusEffectType.Ignite), 0);
            build.TryEquipCore(AilmentCore("core_fire_b", SkillTag.Fire, StatusEffectType.Ignite), 1);
